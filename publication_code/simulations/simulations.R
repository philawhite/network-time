...
sim_out = mclapply(1:length(cs),function(j){
  ...
  sim_res = mclapply(1:n_sim,function(i){
    ...
    gc_network = optim(par=c(log(0.9),log(cs[j]),log(.2)),fn=fn_optim_gc,control=list(fnscale=-1),y = y)
    gc_euc = optim(par=c(log(0.9),log(cs[j]),log(.2)),fn=fn_optim_gc_euc,control=list(fnscale=-1),y = y)
    dag_network = optim(par=c(log(0.9),log(cs[j]),log(.2)),fn=fn_optim_dag,control=list(fnscale=-1),y = y)
    dag_euc = optim(par=c(log(0.9),log(cs[j]),log(.2)),fn=fn_optim_dag_euc,control=list(fnscale=-1),y = y)
